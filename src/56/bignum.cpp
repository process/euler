...
bignum_ * bigdigitsum(bignum ret, bignum num)
{
	...
	int i;
	for(i = 0; i < num->pos; i++)
		bigaddint(rettemp, rettemp, num->num[i]);

	return bigsetbig(ret, rettemp);
}
...
int biggetint(bignum num)
{
	...
	int i;
	for(i = 0; i<num->pos||power>1000000000; i++, power*=10)
	{
		tempres=res;
		res += num->num[i] * power;

		if(tempres > res) //overflow
		{
			res=tempres;
			break;
		}
